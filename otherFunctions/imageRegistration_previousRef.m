 function [imgAll2,shiftAll,max2]=imageRegistration_previousRef(imgAll,upsampling)
 % register current image against the previous one; imgAll is the stack; 
% parpool('local',16)
imgAll2=double(imgAll);

p=size(imgAll,3);
% h_wait=waitbar(0.5,'please wait...,doing FFT2');
a=upsampling;
shift=zeros(p,2);
imgFFT=fft2(imgAll2);
    shift(2,1)=3;
    ...
    max2=zeros(p,1);
    error2=zeros(p,1);
    while sum(abs(shift(:)))>2 & iteration<=1
...
%             ref2=ref-imgFFT(:,:,ii);
%             ref2=ref2/(p-1);             
%             waitbar(ii/p,h_wait,['Iteration',num2str(iteration),':completed', num2str(ii/p*100,'%04.1f'),'%']);
%             img=double(dataAllTmp{ii});
          [output, imgFFT(:,:,ii),max2(ii)] = dftregistrationIOS(ref2,imgFFT(:,:,ii),a);
%           imgFFT(:,:,ii)=single(img2FFT);
            shift(ii,:)=output(3:4);
            error2(ii)=output(1);
%             imgFFT(:,:,ii)=x;
         end
         shiftAll=shiftAll+shift;
        disp(['Iteration:',num2str(iteration),';Residual shift',num2str(sum(abs(shift)))]);
    end
    ii=1;
    [output, imgFFT(:,:,ii),max2(ii)] = dftregistrationIOS(imgFFT(:,:,ii),imgFFT(:,:,ii),a);
    error2(ii)=output(1);
    shift(ii,:)=output(3:4);
 for ii=1:p
     imgAll2(:,:,ii)=circshift(imgAll(:,:,ii),shiftAll(ii,:));
 end
%  imgFFT=abs(ifft2(imgFFT,'symmetric'));
%  imgAll2=cast(imgFFT,'like',imgAll);
 trash=0;
 max2=1-error2;
 % check whether shift on Fourier plane is equivalent to spatial plane:
 %a=circshift(imgAll(:,:,p),shiftAll(p,:));b=a-imgAll2(:,:,p);max(abs(b(:)))
 
%   close(h_wait)
%   delete(gcp('nocreate'))
 

function [output, Greg,max2] = dftregistrationIOS(buf1ft,buf2ft,usfac)
