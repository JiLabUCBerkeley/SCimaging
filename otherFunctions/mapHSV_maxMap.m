function [rgbImg,map2,discreteMap,discreteMap2]=mapHSV_maxMap(img,angleMap,options)
ratio=0.8; % changed on 05/29/2018 to avoid the same color at two ends
stretchFlag_user=0;
    if nargin>2
...
%      m1=3;n1=3; n1=round(length(mapH(:))/m1);
   for ss=3:5
    ...
    discreteMap=changeOrder(discreteMap);
%%
m1=1;n1=length(mapH(:))/m1;
    ...
    discreteMap2=changeOrder(discreteMap2);
end
function img2=changeOrder(img)
img2=img;
img2(:,:,2)=img(:,:,1);
img2(:,:,1)=img(:,:,2);