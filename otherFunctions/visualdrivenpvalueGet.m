function [pvalue,varargout]=visualdrivenpvalueGet(y,input,method)  
if nargin==2
...
%     framePerSti=input.framePerSti;
    y2=reshape(y,framePerSti_deletemoving,trialNO,angleNO);
    y3=squeeze(mean(y2,1));% first dimension is trialNO and the 2nd dimension is the angleNO
    
     y2=reshape(y,framePerSti_deletestation,trialNO,angleNO);
    y3=squeeze(mean(y2,1));% first dimension is trialNO and the 2nd dimension is the angleNO
