classdef TifStream < handle
    ...
    %   ts = tifStream(filename, frameWidth, frameHeight, bitsPerSample)
    ...
    %   Added bitsPerSample as an input option
    %% *************************************
    ...
    methods
        function obj = TifStream(filename, frameWidth, frameHeight, bitsPerSample, varargin)
            
            %Process input arguments
            obj.imageDescription = '';
            propNames = {};
            propVals = {};
            if nargin < 4, bitsPerSample = 16; end
            if ~isempty(varargin)
